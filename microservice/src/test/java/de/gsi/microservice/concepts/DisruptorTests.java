...
public class DisruptorTests { // NOPMD NOSONAR -- nomen est omen
    ...
    private final int bufferSize = 8192; // Specify the size of the ring buffer, must be power of 2.
    ...
    private static final EventTranslatorOneArg<ByteArrayEvent, byte[]> TRANSLATOR = (event, sequence, bb) -> event.array = bb;
    ...
    public DisruptorTests(int nWorkers) {
        ...
        for (int i = 0; i < nWorkers; i++) {
            ...
            disruptorOut.handleEventsWith((inputEvent, sequence, endOfBatch) -> {
                ...
                //System.out.println("WorkerID: " + threadWorkerID + " sequence ID " + sequence + " - Event: " + ZData.toString(inputEvent.array) + " thread = " + Thread.currentThread().toString());
    ...
    public byte[] receiveMessage() {
        ...
        if (readPosition <= inputBuffer.getCursor()) {
    ...
    //        public void onEvent(ByteArrayEvent event, long sequence, boolean endOfBatch) {
    //            System.out.println("Event: " + ZData.toString(event.array));
    //        }
    //    }
}
